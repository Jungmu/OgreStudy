...
public:
	...
	void moveNinja(const FrameEvent &evt)
	{
		mDirection = mProfessorNode->getPosition() - mNinjaNode->getPosition(); 
		mDistance = mDirection.normalise(); 

		mSrcQuat = mNinjaNode->getOrientation();
		mDestQuat = Vector3(Vector3::NEGATIVE_UNIT_Z).getRotationTo(mDirection);
...
private:
	...
	Vector3 mDirection, mDestination;
	Quaternion mSrcQuat, mDestQuat;
	Real mWalkSpeed, mDistance;
